...
export const AdsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // App Tracking Transparency (ATT): Apple, iOS 14.5+'ta reklam SDK'sının cihaz
      // reklam tanımlayıcısını (IDFA) kullanabilmesi için bu izni ayrıca istememizi
      // ŞART koşuyor — bu, yukarıdaki GDPR/UMP onayından FARKLI bir izin. Apple'ın
      // önerdiği sıra: önce UMP, sonra ATT, en son reklam SDK'sını başlat. Android'de
      // bu API'ler no-op döner (iOS'a özel), o yüzden platform kontrolüne gerek yok.
      try {
        ...
        // reklam SDK'sı ATT reddedilse/atlansa bile non-personalized modda çalışmaya devam eder.
      }
